...
/** The eight field recordings — story beats in collection order is fixed per zone. */
export const TAPE_LOGS: Record<string, TapeLog> = {
  station: {
    zoneId: 'station', title: 'RECORDER 01 — RANGER STATION',
    lines: [
  ...
  mill: {
    zoneId: 'mill', title: 'RECORDER 05 — LOGGING MILL',
    lines: [
  ...
  radio: {
    zoneId: 'radio', title: 'RECORDER 06 — RADIO RELAY',
    lines: [
  ...
  tunnel: {
    zoneId: 'tunnel', title: 'RECORDER 07 — RAIL TUNNEL',
    lines: [
...
export class TapeSystem {
  ...
  /** place one tape per zone from its seeded pool */
  spawnAll(runSeed: number): void {
    ...
    for (const [zoneId, pool] of this.map.tapePools) {
